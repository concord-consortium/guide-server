...
class BKTParameterLearner {
    ...
    runAsync(studentIds) {
        ...
        let url = new URL(process.env.DB_URI);
        let dbName = url.pathname.substr(1);
        let db;
        console.info("Connect to: %s db: %s", process.env.DB_URI, dbName);
        this._recordPerformanceStats("Database", process.env.DB_URI);
        return MongoClient.connect(process.env.DB_URI).then((client) => {
            this.client = client;
